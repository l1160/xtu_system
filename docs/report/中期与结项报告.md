...
结项版建议在导出 Word 时插入以下系统截图：

| 图号 | 页面 | 说明 |
| --- | --- | --- |
| 图4-1 | 登录页 | 展示账号密码登录入口和错误提示 |
| 图4-2 | 工作台 | 展示统计卡片、待办事项和趋势数据 |
| 图4-3 | 用户管理 | 展示用户查询、新增、编辑、状态切换、重置密码和角色分配 |
| 图4-4 | 角色管理 | 展示角色维护和菜单权限分配 |
| 图4-5 | 学生管理 | 展示学生列表、导入、导出、批量删除和创建账号 |
| 图4-6 | 教师管理 | 展示教师列表、账号绑定和权限收敛效果 |
| 图4-7 | 课程管理 | 展示课程维护和附件关联 |
| 图4-8 | 公告管理 | 展示公告发布、撤回和附件关联 |
| 图4-9 | 申请管理 | 展示申请提交、审批状态和流转记录 |
| 图4-10 | 附件管理 | 展示附件列表、下载和删除 |
| 图4-11 | 日志管理 | 展示登录日志和操作日志查询 |

截图文件可统一存放到 `docs/report/assets/`，在 Markdown 中使用相对路径引用，最终导出为 docx 时插入正文。
